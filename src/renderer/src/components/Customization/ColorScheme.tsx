...
const ColorScheme = () => {
  ...
  const { presetColor } = useAppSelector((state) => state.config.config);
  ...
  const handlePresetColorChange = (event: ChangeEvent<HTMLInputElement>) => {
    console.log(`Change PresetColor:${event.target.value}`);
    dispatch(
      saveConfigToLocalStorage({ key: 'presetColor', value: event.target.value as PresetColor })
    );
  };
